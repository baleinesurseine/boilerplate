...
terminalContainer.className += ' fade-in'
...
function resetSocket () {
  terminalContainer.className = terminalContainer.className.replace(/\bfade-in\b/, 'fade-out')
  ...
  fetch('/terminals?cols=' + term.cols + '&rows=' + term.rows + '&processID=' + pid, {method: 'GET'}).then(function (res) {
    ...
    res.text().then(function (pid) {
      ...
      socket = new WebSocket(socketURL)
      terminalContainer.className = terminalContainer.className.replace(/\bfade-out\b/, 'fade-in')
